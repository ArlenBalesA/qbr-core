...
function QBCore.Player.CheckPlayerData(source, PlayerData)
    ...
    PlayerData.metadata["bloodtype"] = PlayerData.metadata["bloodtype"]
        or QBCore.Config.Player.Bloodtypes[math.random(1, #QBCore.Config.Player.Bloodtypes)]
    ...
    PlayerData.metadata["mining"] = PlayerData.metadata["mining"] or 0
...
function QBCore.Player.CreatePlayer(PlayerData, Offline)
    ...
    self.Functions.AddLevel = function(level, amount)
        ...
        if self.PlayerData.metadata['levels'][level] and amount > 0 then
            ...
            self.Functions.UpdatePlayerData()
            TriggerEvent('qbr-log:server:CreateLog', 'levels', 'AddXp', 'lightgreen', '**'..GetPlayerName(self.PlayerData.source) .. ' [CitizenID: '..self.PlayerData.citizenid..' | id: '..self.PlayerData.source..']** got '..level..' levels '..amount..' [current level: '..self.PlayerData.metadata['levels'][level])
            return true
        end
        return false
    end
    ...
    self.Functions.RemoveLevel = function(level, amount)
        ...
        if self.PlayerData.metadata['levels'][level] and amount > 0 then
            ...
            self.Functions.UpdateLevelData(level)
            self.Functions.UpdatePlayerData()
            TriggerEvent('qbr-log:server:CreateLog', 'levels', 'RemoveXp', 'lightgreen', '**'..GetPlayerName(self.PlayerData.source) .. ' [CitizenID: '..self.PlayerData.citizenid..' | id: '..self.PlayerData.source..']** lost '..level..' levels '..amount..' current level: '..self.PlayerData.metadata['levels'][level])
            return true
        end
        return false
    end
    ...
    function self.Functions.RemoveMoney(moneytype, amount, reason)
        ...
        if not self.Offline then
            ...
            if moneytype == "bank" then
                TriggerClientEvent("qbr-phone:client:RemoveBankMoney", self.PlayerData.source, amount)
            end
            TriggerClientEvent(
    ...
    function self.Functions.GetCardSlot(cardNumber, cardType)
        ...
        local slots = exports["qbr-inventory"]:GetSlotsByItem(self.PlayerData.items, item)
...
function QBCore.Player.SaveOffline(PlayerData)
    if PlayerData then
        ...
        if GetResourceState("qbr-inventory") ~= "missing" then
            exports["qbr-inventory"]:SaveInventory(PlayerData, true)
        end
...
function QBCore.Player.GetTotalWeight(items)
    ...
    return exports["qbr-inventory"]:GetTotalWeight(items)
end
...
function QBCore.Player.GetSlotsByItem(items, itemName)
    ...
    return exports["qbr-inventory"]:GetSlotsByItem(items, itemName)
end
...
function QBCore.Player.GetFirstSlotByItem(items, itemName)
    ...
    return exports["qbr-inventory"]:GetFirstSlotByItem(items, itemName)
end
